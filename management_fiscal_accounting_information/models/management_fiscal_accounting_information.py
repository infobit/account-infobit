      ...
      if self.state == 'draft':
        if self.tax_ids:
         for rec in self:
          ...
          self._cr.execute("SELECT t.type_tax_use as type, sum(it.base) as base, round(sum(round(it.amount,3)),2) as cuota FROM account_tax t inner join account_invoice_tax it on t.id = it.tax_id inner join account_invoice i on it.invoice_id = i.id and " +
"(i.date_invoice between %s and %s) inner join type_taxes_information tti on t.type_taxes_information_id = tti.id and tti.exclude = 'n'  where t.id in %s group by t.type_tax_use", (self.date_start, self.date_end, tuple(taxes),))
          ...
          for type in types:
           ...
           self._cr.execute("SELECT tax.type_taxes_information_id as group, sum(it.base) as base, round(sum(round(it.amount,3)),2) as cuota FROM account_invoice_tax it inner join account_invoice i on it.invoice_id = i.id inner join account_tax tax " +
...
"group by tax.type_taxes_information_id", (tuple(taxes), self.date_start, self.date_end, type['type'], ))
           ...
           for group in groups:
            if journals:
              if partners:
               self._cr.execute("SELECT it.tax_id as impuesto, min(tax.name) as nombre, i.type as type, tti.id as group, sum(it.base) as base, round(sum(round(it.amount,3)),2) as cuota, tti.exclude_amount_untaxes as exclude_base FROM account_invoice_tax " +
...
"group by it.tax_id, tti.exclude_amount_untaxes, i.type order by it.tax_id, i.type", (group['group'], tuple(taxes), self.date_start, self.date_end, tuple(partners), tuple(journals), ))
               ...
               self._cr.execute("SELECT it.tax_id as impuesto, min(tax.name) as nombre, i.type as type, tt.id as group, sum(it.base) as base, round(sum(round(it.amount,3)),2) as cuota, tti.exclude_amount_untaxes as exclude_base FROM account_invoice_tax " +
"it inner join account_invoice i on it.invoice_id = i.id inner join account_tax tax on it.tax_id = tax.id and tax.type_taxes_information_id = %s inner join type_taxes_information tti on tax.type_taxes_information_id = tti.id where " +
            ...
            for ele in data:
                ...
                if reg:
                 ...
                 if reg.type == 'Venta' or reg.type == 'Abono venta':
                  self._cr.execute("SELECT ti.tax_id as impuesto, t.name as nombreimpuesto, ti.base as base, round(round(ti.amount,3),2) as cuota, ty.type_tax_use as type_tax_use from account_invoice_tax ti "+
...
"as partner, i.amount_untaxed as baseinvoice, rp.name as empresa, rp.vat as cif, i.amount_total as total, t.name as nombreimpuesto, ti.base as base, round(round(ti.amount,3),2) as " +
